...
class FlowStore {
  ...
  routes = {
    ...
    getOne: (id: string): Route[] | false => {
      const wantedRoutes = this.flow?.payload?.routes.filter(
        (route) => route.id === id,
      );
    ...
    filter: (options: {
      ...
      let wantedRoutes = this.flow?.payload?.routes.filter(
        (route) => route.path === String(path || route.path),
      );

      if (method) {
        wantedRoutes = wantedRoutes.filter((route) => route.methods[method]);
      }
    ...
    areChanged: () => {
      ...
      return JSON.stringify(originalRoutes) === JSON.stringify(latestRoutes)
        ? false
        : true;
    },
    ...
    editRoute: (options: { id: string; newPath: string }) => {
      ...
      this.makeChange(() => {
        ...
        payload.routes[payload.routes.indexOf(wantedRoute[0])].updatedAt =
          Date.now();
    ...
    addMethod: (routeId: string, method: HttpMethod) => {
      ...
      if (exist) {
        return pushError(
          `${method} method already exist on ${wantedRoute[0].path}`,
        );
      }
...
const session = {
  set: (flow: Flow, date?: number): { success: boolean; data?: Flow } => {
    try {
      ...
      localStorage.setItem(`flow-session-${flow.id}`, JSON.stringify(flow));
      return { success: true };
    } catch (err: unknown) {
...
const origin = {
  set: (flow: Flow, date?: number): void => {
    try {
      flow.stateUpdated = date || Date.now();
      sessionStorage.setItem(`flow-origin-${flow.id}`, JSON.stringify(flow));
      return;
    } catch (err: unknown) {
      return;
    }
