  ...
  return (
    <div className="p-5 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 flex gap-5">
      {workspaces.map((workspace) => (
        <div
          key={`workspaceCard-${workspace?.id}`}
          className="p-5 border-1 rounded-xl cursor-pointer hover:bg-[#28282828] relative"
        >
          <Link
          ...
          >
            <img
              src={`${imgSrc}${workspace.id}`}
              className="w-10 border-1 rounded-lg p-2"
            />
            <div className="flex flex-col gap-0.5 mt-1">
              <Text className="font-semibold opacity-90" size="3">
                {workspace.name}
              </Text>
            ...
            <DropdownMenu.Root>
              ...
              >
                ...
                <DropdownMenu.Sub>
                  ...
                  <DropdownMenu.SubContent className="bg-black">
                    <MenuOption
                      icon="link"
                      title="All domains"
                      path={`/workspaces/${workspace.id}/domains`}
                    />
                    <MenuOption
                      icon="plus"
                      title="Connect domain..."
                      path={`/workspaces/${workspace.id}/domains/new`}
                    />
                ...
                <MenuOption
                  icon="database"
                  title="Databases"
                  path={`/workspaces/${workspace.id}/databases`}
                />
                <DropdownMenu.Separator />
                <MenuOption
                  icon="settings"
