    ...
    return (
        ...
        <>
            {Object.entries(tabs).map(([key, tab]) => {
                ...
                const isValid = currentRoute?.methods?.hasOwnProperty(tab.method) || false;
                const isOpen = openTab?.routeId === tab.routeId && openTab?.method === tab.method;
...
function TabItem({ tab, isOpen, onClick, close, isValid }: ItemParams) {
    ...
    if (!isOpen) {
        return (
            <div
                className="editorTab group"
            >
                <button onClick={onClick} className="flex flex-col w-full items-start pl-3 pr-3 truncate">
                    <Text size="1" color={isValid ? "gray" : "red"}>{tab.path}</Text>
                    <Text size="1" color={methodsColors[tab.method] as any}>{tab.method}</Text>
                </button>
                <button
                    onClick={close}
    ...
    return (
        <div
            className="editorTab group border-b-1 border-b-power"
        >
            <button onClick={onClick} className="flex flex-col w-full items-start pl-3 pr-3 truncate">
                <Text size="1" color={isValid ? "gray" : "red"}>{tab.path}</Text>
                <Text size="1" color={methodsColors[tab.method] as any}>{tab.method}</Text>
