...
export default function InputsDialog({ store }: { store: FlowStore }) {
  const [_forceUpdate, setForceUpdate] = useState<boolean>(false);
  ...
  const { param, source, block } = options;
        ...
        >
          <div className="flex w-full min-h-[100%]">
            <div className="w-[25%] p-2 pl-3 pr-3 flex flex-col gap-3">
              <InfoRow name="Block" value={block.data.label} icon="package" />
              <InfoRow name="Source" value={source.name} icon="source-code" />
              <div className="w-full border-t-1 border-border/50 mt-3 mb-3"></div>
              <InfoRow name="Key" value={param.name} icon="square-key" />
              <InfoRow
                name="Type"
                value={type}
            ...
            <div className="w-[75%] min-h-[100%] p-2 pl-4 pr-1 flex flex-col gap-5">
              <InputBody options={options} setOpen={setOpen} store={store} />
            </div>
          </div>
        </DialogContent>
      </Dialog>
...
function InfoRow({ name, value, icon, color }: InfoRowParams) {
  return (
    <div className="flex items-center gap-1">
      ...
      <Text size="1" color={color && (color as any)}>
        {value}
      </Text>
    </div>
  );
