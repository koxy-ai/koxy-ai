...
export default function AddMethod({ store }: { store: FlowStore }) {
  ...
  store.events.addListener("openMethodAdd", "methodAdd", (route: Route) => {
    setRoute(route);
    setMethod("get");
    setOpen(true);
  });
  ...
  return (
    <EditDialog open={open} setOpen={setOpen} icon="plus" color="var(--power)">
      <div className="flex flex-col gap-4 py-4">
        <Select defaultValue={method} onValueChange={setMethod}>
          ...
          <SelectContent className="outline-0">
            {methods.map((method) => (
              <SelectItem key={`option-${method}-${Date.now()}`} value={method}>
                {method}
              </SelectItem>
            ))}
          </SelectContent>
      ...
      <DialogFooter>
        <Button className="powerShadow" variant="power" size="sm" onClick={add}>
          Add method
        </Button>
      </DialogFooter>
    </EditDialog>
